...
async function registrarPagamento() {
    ...
    try {
        // CORREÇÃO 1: Verificar se o crismando existe no Supabase
        const { data: crismandoExiste, error: errorVerificacao } = await supabase
            .from('crismandos')
            .select('id, nome, telefone')
            .eq('id', parseInt(crismandoId))
            .single();

        if (errorVerificacao || !crismandoExiste) {
            console.error('Crismando não encontrado:', errorVerificacao);
            alert('❌ Erro: Crismando não encontrado no banco de dados. Recarregue a página e tente novamente.');
        ...
        // CORREÇÃO 2: Verificar se já existe pagamento para este mês
        const { data: pagamentoExistente, error: errorPagamento } = await supabase
            .from('pagamentos')
            .select('id')
            .eq('crismando_id', parseInt(crismandoId))
            .eq('mes', mes);

        if (errorPagamento) {
            console.error('Erro ao verificar pagamento existente:', errorPagamento);
        }

        if (pagamentoExistente && pagamentoExistente.length > 0) {
            const confirmar = confirm(`⚠️ Já existe um pagamento registrado para ${crismandoExiste.nome} no mês ${mes}.\n\nDeseja continuar mesmo assim?`);
            if (!confirmar) {
                return;
            }
        }

        // CORREÇÃO 3: Inserir pagamento com validação
        const dadosPagamento = {
            crismando_id: parseInt(crismandoId),
            mes: mes.trim(),
            valor: parseFloat(valor)
        };

        console.log('Inserindo pagamento:', dadosPagamento);
        ...
        if (error) {
            console.error('Erro detalhado ao inserir pagamento:', error);
            throw error;
        }

        // Atualizar array local
        pagamentos.push(data[0]);
        atualizarTabela();
        atualizarEstatisticas();
        ...
        if (error.code === '23503') {
            alert('❌ Erro: O crismando selecionado não existe no banco de dados.\n\n🔄 A página será recarregada para sincronizar os dados.');
            location.reload();
        } else if (error.code === '23505') {
            alert('❌ Erro: Já existe um pagamento registrado para este crismando neste mês.');
        } else {
...
function loadExcel() {
    ...
    if (!fileName.endsWith('.xlsx') && !fileName.endsWith('.xls')) {
        alert('Formato de arquivo inválido. Por favor, selecione um arquivo .xlsx ou .xls');
        return;
    }

    // Verificar se XLSX está carregado
    if (typeof XLSX === 'undefined') {
        alert('Erro: Biblioteca XLSX não carregada. Recarregue a página e tente novamente.');
        return;
    }
    ...
    reader.onload = function(e) {
        try {
            ...
            console.log('Arquivo lido com sucesso. Planilhas encontradas:', workbook.SheetNames);
            ...
            jsonData.forEach((row, index) => {
                ...
                const telefone = row.Telefone || row.telefone || row.TELEFONE ||
                               ...
                               row.Fone || row.fone || row.FONE || '';
                
                const valor = parseFloat(
                    row.Valor || row.valor || row.VALOR ||
                    row['Valor Mensal'] || row['valor mensal'] ||
                    row.Mensalidade || row.mensalidade || 0
                );
                
                const mes = row.Mes || row.mes || row.MES ||
                           row.Mês || row.mês || row.MÊS || '';

                console.log(`Linha ${index + 1}:`, {nome, telefone, valor, mes});

                if (nome && nome.trim()) {
                    // Verificar se já existe
                    const existe = crismandos.find(c => 
            ...
            if (error.message.includes('XLSX')) {
                alert('Erro: Biblioteca XLSX não foi carregada corretamente. Recarregue a página e tente novamente.');
            } else if (error.message.includes('Unsupported file')) {
                alert('Erro: Arquivo não suportado. Certifique-se de que é um arquivo Excel válido (.xlsx ou .xls).');
            } else {
...
function processarDadosExcel(jsonData) {
    ...
    rows.forEach((row, index) => {
        ...
        const nome = extrairValor(row, headers, ['nome', 'Nome', 'NOME', 'Nome Completo']);
        const telefone = extrairValor(row, headers, ['telefone', 'Telefone', 'TELEFONE', 'celular', 'Celular']);
        const valor = extrairValorNumerico(row, headers, ['valor', 'Valor', 'VALOR', 'Valor Mensal', 'mensalidade']);
...
function extrairValor(row, headers, possiveisNomes) {
    for (const nome of possiveisNomes) {
        const index = headers.findIndex(h => 
            h && h.toString().toLowerCase().trim() === nome.toLowerCase()
        );
...
pagamentosParaSalvar.forEach(item => {
    const crismandoEncontrado = crismandosSalvos.find(c => 
